...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>ATMons &mdash; Stats</title>
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ccircle cx='50' cy='50' r='50' fill='%2358a6ff'/%3E%3Ctext x='50' y='68' font-size='58' text-anchor='middle' fill='white' font-family='sans-serif' font-weight='bold'%3EA%3C/text%3E%3C/svg%3E">
<style>
""" + _HEAD_CSS + """
main{max-width:1100px;margin:0 auto;padding:24px}
.stats-hero{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:20px}
.card{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:20px}
.card-label{font-size:.72rem;color:#8b949e;text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px}
.card-value{font-size:2rem;font-weight:700;color:#e6edf3;line-height:1}
.card-sub{margin-top:10px;color:#8b949e;font-size:.82rem}
.section{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:20px;margin-bottom:20px}
.section-title{font-size:.72rem;font-weight:600;color:#8b949e;text-transform:uppercase;letter-spacing:.07em;margin-bottom:14px}
.source-box{background:#0d1117;border:1px solid #21262d;border-radius:8px;padding:14px;color:#c9d1d9;font-family:'Consolas','Courier New',monospace;font-size:.82rem;line-height:1.55;word-break:break-word}
.note{font-size:.84rem;color:#8b949e;line-height:1.6}
.actions{display:flex;gap:10px;flex-wrap:wrap}
.btn{padding:9px 18px;border:none;border-radius:6px;cursor:pointer;font-size:.85rem;font-weight:500;display:inline-flex;align-items:center;gap:7px;transition:opacity .15s,transform .1s}
.btn:hover:not(:disabled){opacity:.85}
.btn:active:not(:disabled){transform:scale(.97)}
...
<main>
  ...
  <div class="section" id="statsActionsSection">
    ...
    <div class="actions">
      ...
      <button class="btn btn-green" onclick="quickCmd('whitelist off')">Whitelist Off</button>
      <button class="btn btn-green" onclick="quickCmd('bossbar remove watchdog:restart')">Clear Restart Bar</button>
    </div>
  </div>
...
function _applyServerRole(sid){
  const role=_serverRoles[sid];
  const isViewer=(role!=='admin'&&role!=='owner');
  const as=document.getElementById('statsActionsSection');
  if(as) as.style.display=isViewer?'none':'';
}
function selectServer(sid){
  currentSid=sid;
...
async function poll(){
  try{
    ...
    if(r.status===401){window.location='/login';return;}
    const data=await r.json();
    if(data.error){toast('Stats load failed: '+data.error,'err');return;}
    updateStatus(data.status);
...
async function quickCmd(cmd){
  try{
    const r=await fetch('/server/'+currentSid+'/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:cmd})}).then(r=>r.json());
    if(r.ok) toast('Sent: '+cmd,'ok');
